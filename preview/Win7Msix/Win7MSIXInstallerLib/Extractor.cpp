...
HRESULT Extractor::CreateHandler(MsixRequestImpl * msixRequest, IPackageHandler ** instance)
{
...
HRESULT Extractor::NeedToCopyFile(std::wstring sourceFullPath, std::wstring targetFullPath, _Out_ bool &needToCopyFile)
{
	...
	std::wstring targetVersionString = Win7MsixInstaller_ConvertVersionToString(targetFileVersion);
	std::wstring sourceVersionString = Win7MsixInstaller_ConvertVersionToString(sourceFileVersion);
...
HRESULT Extractor::RemoveVfsFile(std::wstring fileName)
{
	...
	Win7MsixInstaller_GetPathParent(fullPath);
...
HRESULT Extractor::ConvertVfsNameToFullPath(std::wstring fileName, std::wstring& fileFullPath)
{
	...
	for (auto& pair : map)
	{
		...
		{
			Win7MsixInstaller_GetPathChild(remainingFilePath); // remove the FirstDir directory.
